...
router.post('/', async (request, response) => {
    try {
        ...
        kamran.database.editEntry("b59b7631-f435-4189-b2ab-0c26a27def85",user.id, user)

        const clinetKey = uuidv4();
        const ip = request.ip;
        const created = Date.now();
        const userId =user.id;
        kamran.sessions.createSession(clinetKey, ip, created, userId)
