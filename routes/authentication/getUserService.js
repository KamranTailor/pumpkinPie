...
router.post('/', async (request, response) => {
    try {
        const { email, clientKey, service } = request.body;  // Fix typo
        ...
        const serviceDta = await kamran.activity.setActivity(user.id, service)
        const { password, ...safeUser } = user;
        return response.json({ status: true, message: 'Session Correct', user: safeUser, service: serviceDta });
